    ...
    public sealed partial class FmMain
    {
        ...
        public void trans_Calculate()
        {
            ...
            else
            {
                ...
                if (IniHelper.GetValue("配置", "翻译接口") == "有道")
                {
                    googleTranslate_txt = Translate_YouDao(typeset_txt);
                }
            }
            PictureBox1.Visible = false;
        ...
        public void 翻译文本()
        {
            if (IniHelper.GetValue("配置", "快速翻译") == "True")
            {
                ...
                try
                {
                    ...
                    if (IniHelper.GetValue("配置", "翻译接口") == "有道")
                    {
                        data = Translate_YouDao(trans_hotkey);
                    }
        ...
        private string Translate_YouDao(string strTrans)
        {
            var text = "";
            try
            {
                string param = string.Concat("i=", strTrans, "&from=AUTO&to=AUTO&smartresult=dict&client=fanyideskweb&doctype=json&version=2.1&keyfrom=fanyi.web&action=FY_BY_CLICKBUTTION");
                string cookie = "OUTFOX_SEARCH_USER_ID_NCOO=1705068807.9247787; OUTFOX_SEARCH_USER_ID=\"-1078764186@10.169.0.84\"; _ga=GA1.2.175576901.1603603757; JSESSIONID=aaaJWHQmcz7C7d_YcHyyx; fanyi-ad-id=93873; fanyi-ad-closed=1; ___rl__test__cookies=1606729315108";
                string lts, salt, sign;
                lts = Convert.ToInt64(Convert.ToDouble((DateTime.Now - new DateTime(1970, 1, 1, 0, 0, 0, 0)).TotalSeconds) * 1000).ToString();
                salt = lts + new Random().Next(0, 9).ToString();
                string sign_spile = string.Concat("fanyideskweb", strTrans, salt, YoudaoHelper.GetMD5LastString());
                byte[] inputBye = Encoding.GetEncoding("utf-8").GetBytes(sign_spile);
                sign = BitConverter.ToString(new MD5CryptoServiceProvider().ComputeHash(inputBye)).Replace("-", "").ToLower();
                param = string.Concat(param, "&lts=", lts, "&salt=", salt, "&sign=", sign);
                string respones = CommonHelper.PostStrData("http://fanyi.youdao.com/translate_o?smartresult=dict&smartresult=rule", param, cookie, "http://fanyi.youdao.com/");
                JObject res = JObject.Parse(respones);
                text = (string)res["translateResult"][0][0]["tgt"];
            }
            catch (Exception e)
            {
                text = "[有道接口报错]：\r\n1.接口请求出现问题等待修复。" + e.ToString();
            }
            return text;
        }
